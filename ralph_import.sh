...
source "$SCRIPT_DIR/lib/config.sh" 2>/dev/null || RALPH_STATE_DIR=".ralph"
...
CONVERSION_OUTPUT_FILE="$RALPH_STATE_DIR/conversion_output.json"
CONVERSION_PROMPT_FILE="$RALPH_STATE_DIR/conversion_prompt.md"
...
Usage: $0 <source-file> [project-name] [--current-dir]
...
Arguments:
    ...
    --current-dir   Convert PRD in current directory instead of creating new project
...
The command will:
1. Create a new Ralph project (or use current directory with --current-dir)
...
main() {
    ...
    # Parse flags
    for arg in "$@"; do
        if [[ "$arg" == "--current-dir" ]]; then
            use_current_dir=true
        fi
    done

    # Remove --current-dir from args if present
    if [[ "$use_current_dir" == "true" ]]; then
        if [[ "$project_name" == "--current-dir" ]]; then
            project_name=""
        fi
    fi
    ...
    # Handle two modes: new project vs current directory
    if [[ "$use_current_dir" == "true" ]]; then
        log "INFO" "Converting PRD in current directory"

        # Make source file path absolute if relative
        local source_abs_path
        if [[ "$source_file" = /* ]]; then
            source_abs_path="$source_file"
        else
            source_abs_path="$(pwd)/$source_file"
        fi

        # Run conversion using absolute path (no copy needed)
        convert_prd "$source_abs_path" "$project_name"
