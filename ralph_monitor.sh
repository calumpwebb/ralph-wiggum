...
display_status() {
    ...
    if [[ -f "$RALPH_STATE_DIR/progress.json" ]]; then
        local progress_data=$(cat "$RALPH_STATE_DIR/progress.json" 2>/dev/null)
