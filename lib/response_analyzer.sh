...
parse_json_response() {
    local output_file=$1
    local result_file="${2:-$RALPH_STATE_DIR/json_parse_result}"
...
analyze_response() {
    local output_file=$1
    local loop_number=$2
    local analysis_result_file=${3:-"$RALPH_STATE_DIR/response_analysis"}
    ...
    if [[ -f "$RALPH_STATE_DIR/last_output_length" ]]; then
        local last_length=$(cat "$RALPH_STATE_DIR/last_output_length")
    ...
    echo "$output_length" > "$RALPH_STATE_DIR/last_output_length"
...
SESSION_FILE="$RALPH_STATE_DIR/claude_session_id"
