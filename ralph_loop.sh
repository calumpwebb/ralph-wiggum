...
LOG_DIR="$RALPH_STATE_DIR/logs"
DOCS_DIR="$RALPH_STATE_DIR/docs/generated"
STATUS_FILE="$RALPH_STATE_DIR/status.json"
PROGRESS_FILE="$RALPH_STATE_DIR/progress.json"
...
TIMESTAMP_FILE="$RALPH_STATE_DIR/last_reset"
USE_TMUX=false
...
CLAUDE_SESSION_FILE="$RALPH_STATE_DIR/claude_session_id" # Session ID persistence file
CLAUDE_MIN_VERSION="2.0.76"              # Minimum required Claude CLI version

# Session management configuration (Phase 1.2)
# Note: SESSION_EXPIRATION_SECONDS is defined in lib/response_analyzer.sh (86400 = 24 hours)
RALPH_SESSION_FILE="$RALPH_STATE_DIR/session"              # Ralph-specific session tracking (lifecycle)
RALPH_SESSION_HISTORY_FILE="$RALPH_STATE_DIR/session_history"  # Session transition history
...
EXIT_SIGNALS_FILE="$RALPH_STATE_DIR/exit_signals"
...
EOF
    ...
    if [ $exit_code -eq 0 ]; then
        ...
        # Check if the failure is due to prompt/context being too long
            ...
            log_status "INFO" "Resetting session and disabling --continue to clear context..."
            reset_session "context_overflow"
            # Disable --continue for next run to force fresh context
...
main() {
    ...
    while true; do
        ...
        if [ $exec_result -eq 0 ]; then
            ...
            # Context overflow - session was reset, --continue disabled, retry immediately
            update_status "$loop_count" "$(cat "$CALL_COUNT_FILE")" "context_reset" "retrying"
            log_status "INFO" "Session reset complete, retrying with fresh context (--continue disabled)..."
            sleep 2
            # Note: CLAUDE_USE_CONTINUE will be re-enabled after successful execution if desired
        else
